...
/** ディレクトリ名とファイル名をくっつける. fname_cat と違い、\ / を間に付加.
 */
...
FKS_LIB_DECL FKS_FNAME_CHAR*	fname_fullpathSL(FKS_FNAME_CHAR dst[], unsigned size, const FKS_FNAME_CHAR* path, const FKS_FNAME_CHAR* currentDir)
{
	...
	// 作業用の絶対パスを作る.
	{
		...
		if (isAbs && hasDrive) {	// ドライブ付き絶対パスなら、そのまま.
			...
			if (fname_hasDrive(currentDir)) {	// 絶対パスだけどドライブがない場合はcurrentDirからドライブだけいただく.
				fname_getDrive(wk, wkSz, currentDir);
			}
			fname_cat(wk, wkSz, path);
		} else {
			if (hasDrive) {			// ドライブ付き相対パスで、
				if (!fname_hasDrive(currentDir))	// カレント側にドライブがなければ
					fname_getDrive(wk, wkSz, path);	// pathのドライブ名を設定. ちがえばカレント側のドライブ名になる.
			}
	...
	// "." や ".." を取り除く.
	{
		...
		FKS_FNAME_CHAR*		s	  = fname_skipDrive(wk);	// ドライブ名は弄らないのでスキップ.
		FKS_FNAME_CHAR*		d	  = s;
...
{
	enum { CHECK_MAX_PATH = sizeof(char[FNAME_MAX_PATH >= 16 ? 1 : -1]) };	// コンパイル時のサイズチェック.
	enum { CHECK_MAX_URL  = sizeof(char[FNAME_MAX_URL  >= 16 ? 1 : -1]) };	// コンパイル時のサイズチェック.
